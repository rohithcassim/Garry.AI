...
def _migrate_chat_messages_fts():
    ...
    try:
        conn = sqlite3.connect(db_path)
        try:
            conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS temp._garry-ai_fts5_probe USING fts5(content)")
            conn.execute("DROP TABLE IF EXISTS temp._garry-ai_fts5_probe")
        except Exception as e:
